...
namespace event_manager {
  ...
  struct Event {
    ...
    // Params params; is the ideal way to generalize experiments
    // BUT it raises a variety of issues:
      // event carries generic params -> implies templatization
      // StoreHouse<T>: vector<event<T>> fine
      // but Tracker: vector<StoreHouse<T>> NOT fine
    // Work-arounds: 1\ runtime runtime-polymorhism 2\ addition of new rules
      // Base_event (time_elapsed ONLY) & Specific_event: virtual inherit/CRTP
      // fixed MAX_PARAM_COUNT & COMMON_PARAMS_DATA_TYPE (analysis needs numbers)
      // Stick with 2: Alter params as needed
